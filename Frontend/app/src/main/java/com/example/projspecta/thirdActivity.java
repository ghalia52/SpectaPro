            ...
            public void onResponse(Call<List<String>> call, Response<List<String>> response) {
                if (response.isSuccessful() && response.body() != null) {
                    ...
                    if (villes != null && !villes.isEmpty()) {
                        ArrayAdapter<String> adapter = new ArrayAdapter<>(thirdActivity.this,
                        ...
                        fetchSpectacles(villes.get(0));
                    } else {
                        Toast.makeText(thirdActivity.this, "Aucune ville disponible", Toast.LENGTH_SHORT).show();
                    }
                } else {
                    Toast.makeText(thirdActivity.this, "Erreur lors de la récupération des villes: " + response.code(), Toast.LENGTH_SHORT).show();
                }
            }

            @Override
            public void onFailure(Call<List<String>> call, Throwable t) {
                Toast.makeText(thirdActivity.this, "Erreur réseau: " + t.getMessage(), Toast.LENGTH_SHORT).show();
                ...
                if (response.isSuccessful() && response.body() != null) {
                    ...
                    if (spectacles != null && !spectacles.isEmpty()) {
                        ...
                        if (!filteredSpectacles.isEmpty()) {
                            adapter = new SpectacleAdapter(thirdActivity.this, filteredSpectacles);
                            recyclerView.setAdapter(adapter);
                        } else {
                            Toast.makeText(thirdActivity.this, "Aucun spectacle trouvé pour " + city, Toast.LENGTH_SHORT).show();
                        }
                    ...
                    Toast.makeText(thirdActivity.this, "Erreur lors de la récupération des spectacles: " + response.code(), Toast.LENGTH_SHORT).show();
                }
            }
    ...
    private void searchSpectacles(String query) {
        apiService.universalSearch(query).enqueue(new Callback<List<Spectacle>>() {
            ...
            public void onResponse(Call<List<Spectacle>> call, Response<List<Spectacle>> response) {
                if (response.isSuccessful() && response.body() != null) {
                    List<Spectacle> spectacles = response.body();
                    adapter = new SpectacleAdapter(thirdActivity.this, spectacles);
                    recyclerView.setAdapter(adapter);
                } else {
                    Toast.makeText(thirdActivity.this, "Aucun résultat trouvé.", Toast.LENGTH_SHORT).show();
                }
            }

            @Override
            public void onFailure(Call<List<Spectacle>> call, Throwable t) {
                Toast.makeText(thirdActivity.this, "Erreur lors de la recherche: " + t.getMessage(), Toast.LENGTH_SHORT).show();
        ...
        apiService.universalSearch(date).enqueue(new Callback<List<Spectacle>>() {
            ...
            public void onResponse(Call<List<Spectacle>> call, Response<List<Spectacle>> response) {
                if (response.isSuccessful() && response.body() != null) {
                    ...
                    Toast.makeText(thirdActivity.this, "Aucun résultat trouvé pour cette date.", Toast.LENGTH_SHORT).show();
                }
            }
